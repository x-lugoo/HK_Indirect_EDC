...
#ifdef _S_SERIES_
		else if (ucKey>=KEYATM1 && ucKey<=KEYATM4 )
		{
			ucSelNo = ucPageNo*ucMenuLines + ucKey - KEYATM1;
			break;
		}
//		else if ((ucMode & MENU_UNSEL) && ucKey>=KEY1 && ucKey<=KEY4 )
//		{
//			ucSelNo = ucPageNo*ucMenuLines + ucKey - KEY1;
//			break;
//		}
#endif
#ifdef _P80_
...
uchar PubGetAmount(uchar *pszPrefix, uchar ucDeciPos,
				  ...
				  uchar ucTimeout, uchar ucMisc, uchar ch)
{
	...
	if(ch == 'a')//2015-11-12 allow zero amt
	{
		iZeroFlag = 1;
		ch = 0;
	}
	...
	{	
		...
		/* 矪瞶龄 */
		if(iZeroFlag)//2015-11-12
		{
			if(chUsedTag == 0 && (ch >= KEY0 && glEdcMsgPtr->KeyValue<=KEY9))//2013-12-9 not allow '0'
			{
				ucKeyVal = ch;
				chUsedTag = 1;
			}
		...
		{
		case KEY0:
			if( (pszNowChar==szBuff || pszNowChar>=szBuff+ucMaxLen) && iZeroFlag==0)//2015-11-12
			{
				PubBeepErr();
			}
			else
...
uchar PubDebugOutput(uchar *pszTitle, uchar *psData, ushort nDataLen, uchar ucDevice, uchar ucMode)
{
	...
	if (ucDevice&DEVICE_COM1 || ucDevice&DEVICE_COM2)
	{
		...
		ucPort = (ucPort==1 ? 3 : 0); // S80使用PINPAD口作为第二个COM口
#endif
		//if (PortOpen(ucPort, "9600,8,n,1") != 0)
...
//				UnitSend(psData+usCurrLen, usUnitLen, ucDevice, ucMode, 0);
	...
	if (ucMode&ISO_MODE)
	{
		...
		if (pstDataAttr == NULL)
		{
			if(ChkIfIndirectCupAcq() && glProcInfo.stTranLog.ucTranType == SETTLEMENT)  //added bu jeff_xiehuan20170331
			{
				DebugDefaulDef[60].eElementAttr = Attr_n;
				/*DebugDefaulDef[60].eLengthAttr = Attr_fix;
				DebugDefaulDef[60].uiLength = 11;*/
			}
			else
			{
				DebugDefaulDef[60].eElementAttr = Attr_a;
				DebugDefaulDef[60].eLengthAttr = Attr_var2;
				DebugDefaulDef[60].uiLength = 999;
			}
...
/*----------2014-6-3 KMS START-----------*/
uchar BabySelectOneTwo(uchar *pszItem,uchar *pszChoice1,uchar *pszChoice2)
{
	uchar ucKey, szBuf[20];

	ScrClrLine(2,7);
	PubDispString(pszItem, DISP_LINE_LEFT|2);
	sprintf((char *)szBuf, "[1].%s  [2].%s", pszChoice1,pszChoice2);
	PubDispString(szBuf, DISP_LINE_LEFT|6);
...
//add by richard 20161107, for debug use.
uchar PubDebugTx(uchar *psStr, ...)
{   
  uchar ucRet=0;
  int i;
  ...
  //ucRet = PortOpen(0, "9600,8,n,1");
  ucRet = PortOpen(0,"115200,8,N,1");
  va_start(ap, psStr);       
  vsprintf(buffer,psStr, ap);  
  va_end(ap);  
 ...
 // {
	//PortSend(0,buffer[i]);
 // }
  ucRet = PortSends(0, buffer, strlen(buffer)+1);  //modified by jeff_xiehuan_20170321
  //DelayMs(100);
  PortSend(0,'\r');
  PortSend(0,'\n');
  DelayMs(20);
...
//raymond 3 Jun 2010
  #ifdef IP_INPUT
//************************************************************************************************
//Input IP address without typing the dot '.'
//************************************************************************************************
uchar PubGetIPString(uchar ucMode, uchar ucMinlen, uchar ucMaxlen, uchar *pszOut, uchar ucTimeOut)
{
	uchar	ucKey, bClearInData, szWorkBuff[LEN_GETSTRING+1];
	int		iLen, iIndex, iLastKey;
 
  int dotCnt, ipModuleLen;
  int dotFlag, i;
  ...
  dotCnt = 0;
    ...
    //retrieve the ipModule length and dot Count if the input buffer is not empty, e.g. store the previous IP setting
    dotFlag = FALSE;
    for(i=iLen-1; i>=0; i--)
    {
      if(szWorkBuff[i] == '.')
      ...
      else if(dotFlag == FALSE && ipModuleLen < 3)
      {
        ipModuleLen++;
      }
    }  
  ...
  while( 1 )
	{
      ...
      //add dot '.' to the IP address 
      if(ipModuleLen > 0 && dotCnt < 3)
      {
        szWorkBuff[iLen++] = '.';
        szWorkBuff[iLen]   = 0;          
        ipModuleLen = 0;
      ...
      //end add dot
        ...
        //when clear the previous buffer, the ipModuleLen and doCnt should be reset
        ipModuleLen = 0;
        dotCnt = 0;      
      }
        ...
        //update ipModule len, dot count
        ipModuleLen++;
        if(ipModuleLen >= 3 && dotCnt < 3)
        {
  				szWorkBuff[iLen++] = '.';
	  			szWorkBuff[iLen]   = 0;          
          ipModuleLen = 0;
		...
		{
			bClearInData = FALSE;
      dotFlag = FALSE;        //flag to show the occurrence of dot '.'
      
      if( iLen<=0 )
			{
        //reset dot count and ipModule length if the IP input is empty
        dotCnt = 0;
        ipModuleLen = 0;
				continue;
			}

      //delete the dot and update dot count
      ...
      //end update
			...
			if( iLen<=0 )
			{
        //reset dot count and ipModule length if the IP input is empty after deleting the dot '.'
        dotCnt = 0;
        ipModuleLen = 0;
        continue;
			}
