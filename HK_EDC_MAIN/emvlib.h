...
typedef struct{
	unsigned char MerchName[256];    //坝め
	unsigned char MerchCateCode[2];  //坝め摸絏(⊿璶―ぃ砞竚)
	unsigned char MerchId[15];       //坝め夹粁(坝め腹)
	unsigned char TermId[8];         //沧狠夹粁(POS腹)
	unsigned char TerminalType;      //沧狠摸
	unsigned char Capability[3];     //沧狠┦
	unsigned char ExCapability[5];   //沧狠耎甶┦
	unsigned char TransCurrExp;      //ユ砯刽絏计
	unsigned char ReferCurrExp;      //把σ砯刽计
	unsigned char ReferCurrCode[2];  //把σ砯刽絏
	unsigned char CountryCode[2];    //沧狠瓣產絏
	unsigned char TransCurrCode[2];  //ユ砯刽絏
	unsigned long ReferCurrCon;      //把σ砯刽絏㎝ユ絏锣传玒计(ユ砯刽癸把σ砯刽蹲瞯*1000)
	unsigned char TransType;         //讽玡ユ摸(EMV_CASH EMV_GOODS EMV_SERVICE EMV_GOODS&EMV_CASHBACK EMV_SERVICE&EMV_CASHBACK)
	unsigned char ForceOnline;       //坝め眏羛诀(1 ボ羆琌羛诀ユ)
	unsigned char GetDataPIN;        //盞絏浪代玡琌弄刚Ω计
	unsigned char SurportPSESel;     //琌やPSE匡拒よΑ
}EMV_PARAM;
