...
int CommInitLAN(COMM_CONFIG *pstCfg)//2014-9-17 here for static IP, still need do something
{
	int iRet;//2014-9-17 ttt
#ifdef SXX_IP_MODULE
...
int CommDial(uchar ucDialMode)
{
	...
	case CT_TCPIP:
	    ...
	    break;
	case CT_3G://2015-11-23 3G+SSL
		//iRet = SXX3GDial(&sg_stCurCfg.stWirlessPara, 10/*60*/, 3600, (uchar)(ucDialMode==DM_PREDIAL));//2015-9-14 shorten timeout
		//break;
#if defined(_P80_) || defined(_P90_) || defined(_S_SERIES_)
	case CT_CDMA:
	case CT_GPRS:
        ...
        PubDebugTx("sg_stCurCfg.stWirlessParaszAPN:%s\n",sg_stCurCfg.stWirlessPara.szAPN);
      #endif
...
void CommGetErrMsg(int iErrCode, COMM_ERR_MSG *pstCommErrMsg)
{
	...
	if ((sg_stCurCfg.ucCommType==CT_GPRS) || (sg_stCurCfg.ucCommType==CT_CDMA) ||
		...
		(sg_stCurCfg.ucCommType==CT_3G))//2015-11-23 
	{
...
int RS232RawTxd(uchar *psTxdData, ushort uiDataLen)
{
    ...
    //????
#ifdef _WIN32
    PortOpen(sg_stCurCfg.stRS232Para.ucPortNo,
				(void *)sg_stCurCfg.stRS232Para.szAttr);
#endif
...
#ifdef APP_DEBUG
// 		PrnInit();
// 		PrnStr("Recv Length %d",uiLength);
// 		PrnStr("\n");
// 		PrnStr("Recv Data \n");
// 		for (i=0;i<uiLength;i++)
// 		{
// 			PrnStr("[%02x] ", *psRxdData++);
// 		}	
//	 	PrnStr("\n");
// 		PrnStart();
		DelayMs(300);
