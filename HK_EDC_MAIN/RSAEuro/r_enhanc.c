...
R_RSA_PRIVATE_KEY_CS *privateKey;  /* signer's RSA private key */
{
	int status;
...
unsigned char *signature;       /* signature */
unsigned int signatureLen;      /* length of signature */
R_RSA_PUBLIC_KEY_CS *publicKey;    /* signer's RSA public key */
{
	int status;
	unsigned char digest[MAX_DIGEST_LEN], digestInfo[DIGEST_INFO_LEN],
...
R_RSA_PUBLIC_KEY_CS **publicKeys;  /* public keys */
...
unsigned char iv[8];            /* initialization vector */
R_RSA_PRIVATE_KEY_CS *privateKey;  /* recipient's RSA private key */
{
	int status;
...
R_RSA_PUBLIC_KEY_CS *publicKey;                /* recipient's RSA public key */
R_RSA_PRIVATE_KEY_CS *privateKey;              /* signer's RSA private key */
R_RANDOM_STRUCT *randomStruct;              /* random structure */
{
	R_ENVELOPE_CTX context;
	R_RSA_PUBLIC_KEY_CS *publicKeys[1];
	int status;
